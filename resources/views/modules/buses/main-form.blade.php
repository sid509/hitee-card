<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <div class="input-group input-group-merge">
            <span class="input-group-text"><i class="bx bx-bus"></i></span>
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" 
                value="{{ old('name', $bus->name) }}" placeholder="e.g. City Express" required />
    ...
    <div class="col-md-6 mb-3">
        ...
        <div class="input-group input-group-merge">
            ...
            <input type="text" class="form-control @error('bus_number') is-invalid @enderror" id="bus_number" name="bus_number" 
                value="{{ old('bus_number', $bus->bus_number) }}" {{ auth()->user()->hasRole('super-admin') ? 'required' : 'readonly' }} placeholder="BA 1 PA 1234" />
    ...
    <div class="col-md-6 mb-3">
        ...
        <div class="input-group input-group-merge">
            ...
            <input type="number" class="form-control @error('total_capacity') is-invalid @enderror" id="total_capacity" name="total_capacity" 
                value="{{ old('total_capacity', $bus->total_capacity) }}" required min="0" placeholder="e.g. 40" />
            @error('total_capacity') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
    </div>

    <div class="col-md-6 mb-3">
        <label class="form-label" for="hwid">HWID (Hardware ID)</label>
        <div class="input-group input-group-merge">
            ...
            <input type="text" class="form-control @error('hwid') is-invalid @enderror" id="hwid" name="hwid" 
                value="{{ old('hwid', $bus->hwid) }}" {{ auth()->user()->hasRole('super-admin') ? 'required' : 'readonly' }} placeholder="RFID Hardware ID" />
...
<div class="mb-3">
    <label class="form-label" for="route_id">Assigned Route</label>
    <div class="input-group input-group-merge">
...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        @if($bus->featured_image_url)
            <div class="mb-2">
                <img src="{{ $bus->featured_image_url }}" alt="Featured" class="rounded border" style="width: 100px; height: 100px; object-fit: cover;">
            </div>
        @endif
        <div class="input-group input-group-merge">
    ...
    <div class="col-md-6 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2 mb-2">
            @foreach($bus->media()->where('collection_name', 'gallery')->get() as $image)
                <img src="{{ $image->url }}" alt="Gallery" class="rounded border" style="width: 60px; height: 60px; object-fit: cover;">
            @endforeach
        </div>
        <div class="input-group input-group-merge">
...
<div class="mt-4">
    <button type="submit" class="btn btn-primary me-2"><i class="bx bx-save me-1"></i> Save Bus</button>
    <a href="{{ route('buses.index') }}" class="btn btn-label-secondary">Cancel</a>
</div>
